    ...
    public class CancellationService
    {
        ...
        public async Task<ApiResponse<CancellationResponseDTO>> RequestCancellationAsync(CancellationRequestDTO cancellationRequest)
        {
            try
            {
                ...
                .AsNoTracking()
                .FirstOrDefaultAsync(o => o.OrderId == cancellationRequest.OrderId &&
                ...
                var existingCancellation = await _DbContext.Cancellation.AsNoTracking()
                ...
                _DbContext.Cancellation.Add(cancellation);
                ...
                var cancellationResponse = new CancellationResponseDTO
                {
                    ...
                    OrderAmount = order.TotalAmount,
                    Status = cancellation.CancellationStatus,
        ...
        public async Task<ApiResponse<CancellationResponseDTO>> GetCancellationByIdAsync(int id)
        {
            try
            {
                var cancellation = await _DbContext.Cancellation.AsNoTracking()
        ...
        public async Task<ApiResponse<ConfirmationResponseDTO>> UpdateCancellationStatusAsync(CancellationStatusUpdateDTO statusUpdate)
        {
            ...
            using (var transaction = await _DbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    var cancellation = await _DbContext.Cancellation.Include(c => c.Order).ThenInclude(cust => cust.Customer)
                    ...
                    cancellation.CancellationStatus = statusUpdate.Status;
                    ...
                    {
                        Message = $"Cancellation request with ID {cancellation.Id} has been {cancellation.CancellationStatus}."
                    };
        ...
        public async Task<ApiResponse<List<CancellationResponseDTO>>> GetAllCancellationsAsync()
        {
            try
            {
                var cancellations = await _DbContext.Cancellation
                .AsNoTracking()
                .Include(c => c.Order)
                .ToListAsync();
        ...
        private async Task NotifyCancellationAcceptedAsync(Cancellation cancellation)
        {
            ...
            <h2 style='margin: 0;'>Cancellation Request {cancellation.CancellationStatus}</h2>
            </div>
            <!-- Greeting -->
            <p style='margin: 20px 0 5px 0;'>Dear {cancellation.Order.Customer.FirstName} {cancellation.Order.Customer.LastName},</p>
            <!-- Cancellation Details -->
            <p style='margin: 5px 0 20px 0;'>Your cancellation request for Order <strong>#{cancellation.Order.OrderNumber}</strong> has been <span style='color: #dc3545; font-weight: bold;'>{cancellation.CancellationStatus}</span>.</p>
            ...
            <td style='padding: 8px; border: 1px solid #dddddd;'>{((cancellation.ProcessedAt).HasValue? cancellation.ProcessedAt?.ToString("MMMM dd, yyyy HH:mm") : "N/A")}</td>
            </tr>
            <tr>
            ...
            <td style='padding: 8px; border: 1px solid #dddddd;'>{cancellation.OrderAmount - cancellation.CancellationCharges}</td>
            </tr>
            </table>
            <!-- Footer -->
